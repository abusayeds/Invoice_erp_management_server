...
const permission_model_1 = require("../../make_modules/permission/permission.model");
...
const loginDB = (email, password) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.UserModel.findOne({ email: email, authProvider: "local" }).select('+password');
    if (!user) {
...
const googleLoginDB = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const { email } = payload;
    let user = yield user_model_1.UserModel.findOne({ email: email, authProvider: "google" });
    if (!user) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, "This account does not exist.");
    }
    if (user.isDeleted) {
...
const forgotPasswordDB = (email) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.UserModel.findOne({ email: email, isVerify: true, authProvider: "local" });
    if (!user) {
        throw new AppError_1.default(http_status_1.default.NOT_FOUND, "This account does not exist.");
    }
    const otp = (0, exports.generateOTP)();
...
const allUserDB = (query) => __awaiter(void 0, void 0, void 0, function* () {
    const userQuery = new queryBuilder_1.default(user_model_1.UserModel.find({ role: role_1.role.company }).select('-password -isVerify'), query).sort();
    const { totalData } = yield userQuery.paginate(user_model_1.UserModel.find({ role: role_1.role.company }));
...
const createUserByCompanyDB = (companyId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    const isUserRegistered = yield user_model_1.UserModel.findOne({ email: payload.email });
    if (isUserRegistered) {
        throw new AppError_1.default(http_status_1.default.BAD_REQUEST, "User already exists");
    }
    const rolePermissions = yield permission_model_1.PermissionModel.findOne({
        companyId,
        role: payload.role,
    });
    if (rolePermissions) {
    ...
    payload.companyId = new mongoose_1.Types.ObjectId(companyId);
    payload.isVerify = true; // Auto verify users created by company
