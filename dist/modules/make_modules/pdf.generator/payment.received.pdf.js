...
exports.generatePaymentReceivedPDF = void 0;
...
// ─── Dummy Data ───────────────────────────────────────────────────────────────
const getDummyPaymentReceivedData = () => ({
...
// MAIN GENERATOR
// ════════════════════════════════════════════════════════════════════════════
const generatePaymentReceivedPDF = (settings, res) => __awaiter(void 0, void 0, void 0, function* () {
    const data = getDummyPaymentReceivedData();
    const s = settings || {};
    const style = s.style || {};
    ...
    if (header.header !== false) {
        setFont(true, 16);
        doc.fillColor(rgb(textColor)).text("PAYMENT RECEIVED", margin.left, y, {
        ...
        drawText(`RECEIVED FROM`, infoX, leftY, { color: "#1111" });
        leftY += 11;
        drawText(` ${data.received.name}`, infoX, leftY);
        leftY += 11;
        drawText(`Reg id : ${data.received.regNo}`, infoX, leftY);
        leftY += 11;
        drawText(`Tax ID: ${data.received.taxId}`, infoX, leftY);
        leftY += 11;
...
exports.generatePaymentReceivedPDF = generatePaymentReceivedPDF;
module.exports = { generatePaymentReceivedPDF: exports.generatePaymentReceivedPDF };
