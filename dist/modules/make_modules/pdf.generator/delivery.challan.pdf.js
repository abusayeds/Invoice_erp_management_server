...
exports.generateDeliveryChallanPDF = void 0;
...
// ─── Dummy Data ───────────────────────────────────────────────────────────────
const getDummyDeliveryChallanData = () => ({
    ...
    company: {
        ...
        address: "dhaka\nDhaka\nDhaka 1234 5728\nBangladesh",
        phone: "01770075689",
        mobile: "+8801770075689",
        fax: "25",
        email: "info@invoiic.com",
        website: "https://web.mooninvoice.com",
    },
    billTo: {
        name: "Organization",
        email: "email@moontechiabs.com",
        phone: "7412589633",
        businessPhone: "8523659",
        poBox: "2501",
        taxId: "KT-2030",
        regNo: "REIS 001",
        contactTaxId: "UT147852",
    ...
    products: [
        {
            srNo: 1,
            name: "Moon Invoice Product 1",
            description: "Create FREE unlimited invoices for freelancers, small business owners, and contractors for 7 days.",
            hsn: "HSN0001",
            quantity: 1,
            unitPrice: 160,
            discount: 6,
    ...
    services: [
        {
            srNo: 1,
            name: "TR01 - Moon Invoice Task",
            description: "Create added attachment feature on Invoice, Estimate, Purchase Order, and Credit Note.",
            sac: "SAC0001",
            quantity: 1,
            rate: 120,
            discount: "6%",
        ...
        {
            srNo: 2,
            name: "TR02 - UI Redesign",
            description: "Complete redesign of dashboard and reporting module with new design system.",
            sac: "SAC0002",
            quantity: 2,
            rate: 200,
            discount: "10%",
    ...
    hsnSacSummary: [
        {
            hsnSac: "1116542",
            taxableValue: "100.00 USD",
            centralRate: "0.00",
            centralAmount: "0.00 USD",
            stateRate: "9%",
            stateAmount: "9.00 USD",
            totalTax: "9.00 USD",
        },
        {
            hsnSac: "Total",
...
// MAIN GENERATOR
// ════════════════════════════════════════════════════════════════════════════
const generateDeliveryChallanPDF = (settings, res) => __awaiter(void 0, void 0, void 0, function* () {
    const data = getDummyDeliveryChallanData();
    const s = settings || {};
    const style = s.style || {};
    ...
    const decorateCurrentPage = () => {
        ...
        if (footer.created_moon_invoice_hyperlink !== false) {
            ...
            doc
                ...
                .text("Created by mooninvoice", margin.left, PAGE_H - margin.bottom - 12, { width: CONTENT_W, align: "center" });
        }
        ...
        cells.forEach((cell) => {
            ...
            if (style.horizontal_lines !== "hide") {
                doc
                    ...
                    .strokeColor(rgb(borderColor))
                    .moveTo(cx, yy + rowH)
                    .lineTo(cx + cell.w, yy + rowH)
                    .stroke()
                    .restore();
            }
            setFont(cell.bold || false, baseFontSize - 0.5);
            doc
                .fillColor(rgb(textColor))
                .text(String((_a = cell.value) !== null && _a !== void 0 ? _a : ""), cx + 2, yy + 3, {
                width: cell.w - 4,
    ...
    const drawDescRow = (description, rowIndex, tableX) => {
        ...
        if (style.horizontal_lines !== "hide") {
            doc
                .save()
                .strokeColor(rgb(borderColor))
                .moveTo(tableX, y + descH)
                .lineTo(tableX + CONTENT_W, y + descH)
                .stroke()
                .restore();
        }
    ...
    if (header.header !== false) {
        setFont(true, 16);
        doc.fillColor(rgb(textColor)).text("DELIVERY CHALLAN", margin.left, y, {
            width: CONTENT_W,
            align: header.title_alignment || "center",
        });
        y += 22;
        ...
        if (company.name !== false) {
            ...
            doc
                .fillColor(rgb(textColor))
                .text(data.company.name, infoX, leftY, { width: infoW });
            leftY += 14;
        }
        ...
        detailRows.forEach((row, i) => {
            ...
            drawText(row[1], boxX + boxW * 0.47, ry + 3, {
                width: boxW * 0.5,
                align: "right",
            });
        });
    ...
    // ════════════════════════════════════════════════════════════════════════
    {
        ...
        let billY = y;
        let shipY = y;
        drawText("Delivery Challan To:", billX, billY, { bold: true });
        billY += 12;
    ...
    if (header.sub_title !== false) {
        drawText("Moon Invoice - Easy Invoicing", margin.left, y, {
            bold: true,
            align: header.sub_title_alignment || "center",
            size: baseFontSize + 1,
            width: CONTENT_W,
        });
        y += 14;
    ...
    if (summary.inline_discount !== false)
        sumRows.push([
            "Inline Discount",
            `${data.summary.inlineDiscount.toFixed(2)} USD`,
        ]);
    if (summary.shipping_cost !== false)
        sumRows.push([
            "Shipping Cost",
            `${data.summary.shippingCost.toFixed(2)} USD`,
        ]);
    ...
    if (noteTerms.terms_and_condition !== false) {
        ...
        setFont(false, baseFontSize - 1);
        doc
            .fillColor(rgb("#333333"))
            .text(data.termsAndConditions, termsX, termsY, {
            width: termsW,
            lineBreak: true,
        });
        termsY += 18;
    ...
    if (noteTerms.notes !== false) {
        ...
        doc
            .fillColor(rgb("#333333"))
            .text(data.notes, termsX, termsY, { width: termsW, lineBreak: true });
        termsY += 35;
    }
    sumRows.forEach((row, i) => {
        ...
        drawText(row[0], sumX + 4, sumY + 2, {
            bold: isBold,
            size: baseFontSize - 0.5,
            width: sumW * 0.55,
        });
        drawText(row[1], sumX + sumW * 0.55, sumY + 2, {
            bold: isBold,
            size: baseFontSize - 0.5,
            width: sumW * 0.42,
            align: "right",
    ...
    if (summary.hsc_sac_summary !== false) {
        ...
        hsnCols.forEach((col) => {
            ...
            doc
                .fillColor(rgb(fillTextColor))
                .text(col.label, hx + 2, y + 4, { width: col.w - 4, align: "center" });
            hx += col.w;
        });
        y += 14;
        let hx2 = tableX;
        ...
        data.hsnSacSummary.forEach((row, i) => {
            ...
            vals.forEach((v, vi) => {
                ...
                doc
                    .fillColor(rgb(textColor))
                    .text(v, rx + 2, y + 3, { width: widths[vi] - 4, align: "center" });
                rx += widths[vi];
            });
            y += rh;
        });
    ...
    if (signature.company_sign !== "hide" || header.qr_code !== false) {
        ...
        if (signature.company_sign !== "hide") {
            doc
                .save()
                .strokeColor(rgb(borderColor))
                .moveTo(sigX, baseY + 20)
                .lineTo(sigX + sigW, baseY + 20)
                .stroke()
                .restore();
            drawText(data.signature.companyName, sigX, baseY + 22, {
                bold: true,
                width: sigW,
                align: "center",
            });
            drawText(data.signature.subtitle, sigX, baseY + 35, {
                width: sigW,
                align: "center",
                color: "#666666",
            });
        ...
        if (header.qr_code !== false) {
            try {
                ...
                drawText("QR", qrX + qrW / 2 - 8, baseY + qrW / 2 - 8, {
                    size: 14,
                    bold: true,
                });
            }
...
exports.generateDeliveryChallanPDF = generateDeliveryChallanPDF;
module.exports = { generateDeliveryChallanPDF: exports.generateDeliveryChallanPDF };
