...
export type SupportCrudConfig<T> = {
  model: Model<T>;
  label: string;
...
const withPopulate = (q: any, populate?: SupportCrudConfig<unknown>["populate"]) =>
  populate ? q.populate(populate as PopulateOptions | (string | PopulateOptions)[]) : q;

export const createSupportCrudService = <T>(config: SupportCrudConfig<T>) => {
  const { model, label, perms, searchFields, populate, nameField } = config;
  const ownershipOf = (req: AuthRequest) => resolveSupportOwnership(req, perms.manageAny, perms.manageOwn);
  ...
  const getOwned = async (req: AuthRequest, id: string) => {
    ...
    const base = applyOwnershipToQuery({ _id: id, ...companyScope(companyId) } as FilterQuery<T>, ownership);
  ...
  const create = async (req: AuthRequest, body: Record<string, unknown>) => {
    ...
    if (nameField && payload[nameField] !== undefined) {
      ...
      const dup = await model.findOne({ ...companyScope(companyId), [nameField]: name } as FilterQuery<T>);
