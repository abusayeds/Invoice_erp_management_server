...
/** Base fields shared by every recruitment table (Laravel created_by -> user_id, creator_id, soft delete). */
export const recruitmentBaseFields = {
...
/** Per-company sequential code, e.g. JOB-0001 (Laravel posting_code style). */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const generateSequentialCode = async (
  ...
  const last = await model
    .findOne({ user_id: companyObjectId(companyId), [field]: { $regex: `^${prefix}-` } })
    .sort({ [field]: -1 })
    .lean();
  let next = 1;
  if (last && (last as Record<string, unknown>)[field]) {
    const n = parseInt(String((last as Record<string, unknown>)[field]).split("-").pop() || "0", 10);
