...
const createDB = async (payload: TPurchaseOrder) => {
  ...
  const createdRecord = await PurchaseOrderModel.create(data);
...
const getSingleDB = async (id: string, userId: string) => {
  ...
  if (!record) {
    throw new AppError(httpStatus.NOT_FOUND, 'PurchaseOrder not found');
  }
  return record;
};
...
export const purchaseOrderService = { createDB, getSingleDB, getAllDB };
